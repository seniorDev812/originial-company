...
class ContactFormHandler {
    ...
    init() {
        console.log('ContactFormHandler: Initializing...');
        this.form = utils.getElement('#lead-form');
        this.requirementsTable = utils.getElement('#requirements-table');
        this.addRowBtn = utils.getElement('#add-row-btn');
        this.submitBtn = utils.getElement('#submit-btn');
        this.successMessage = utils.getElement('#success-message');
        
        console.log('ContactFormHandler: Elements found:', {
            form: !!this.form,
            requirementsTable: !!this.requirementsTable,
        ...
        if (this.form) {
            ...
            console.log('ContactFormHandler: Initialization complete');
        } else {
    ...
    bindEvents() {
        console.log('ContactFormHandler: Binding events...');
        this.form.addEventListener('submit', (e) => this.handleSubmit(e));
        
        if (this.addRowBtn) {
            console.log('ContactFormHandler: Adding click event to add-row-btn');
            this.addRowBtn.addEventListener('click', (e) => {
                console.log('ContactFormHandler: Add row button clicked!');
                e.preventDefault();
                this.addTableRow();
            });
        } else {
        ...
        const phoneInput = utils.getElement('#phone');
        if (phoneInput) {
            phoneInput.addEventListener('input', (e) => this.formatPhoneNumber(e.target));
        }
        console.log('ContactFormHandler: Events bound successfully');
    }
    ...
    addTableRow() {
        console.log('ContactFormHandler: Adding table row...');
        if (!this.requirementsTable) {
            console.error('ContactFormHandler: requirementsTable not found!');
            return;
        }

        const tbody = utils.getElement('#requirements-tbody');
        if (!tbody) {
        ...
        console.log('ContactFormHandler: Table row added successfully');
    }
    ...
    removeTableRow(button) {
        ...
        if (row) {
            this.animateRowOut(row, () => {
                row.remove();
                const remainingRows = utils.getElement('#requirements-tbody').querySelectorAll('tr');
    ...
    validateField(field) {
        ...
        if (field.name === 'phone' && value && !utils.validatePhone(value)) {
            isValid = false;
    ...
    validateForm() {
        ...
        const tableRows = utils.getElement('#requirements-tbody').querySelectorAll('tr');
        if (tableRows.length === 0) {
            isValid = false;
            utils.showNotification('Please add at least one product requirement', 'error');
        }
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            utils.showNotification(error.message || 'There was an error submitting your request. Please try again.', 'error');
        } finally {
    ...
    showSuccessMessage() {
        ...
        utils.showNotification('Thank you! Your request has been submitted successfully.', 'success');
    }
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('ContactFormHandler: DOM Content Loaded');
...
window.addEventListener('load', () => {
    console.log('ContactFormHandler: Window Loaded');
