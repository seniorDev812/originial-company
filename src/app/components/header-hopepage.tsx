...
export default function Header({ headerBgColor = 'transparent' }: HeaderProps) {
    ...
    // Logo entrance animation - Step 1: Center to Header
    useEffect(() => {
        const logoField = document.querySelector('.logo-field') as HTMLElement;
        const otherMenus = Array.from(document.querySelectorAll('.other-menu')) as HTMLElement[];
        const Menu = Array.from(document.querySelectorAll('.menu')) as HTMLElement[];
        ...
        if (logoField) {
            ...
            const headerLeft = originalRect.left;
            ...
            setTimeout(() => {
                ...
                // Move straight UP to header's vertical position, stay centered horizontally
                logoField.style.top = `${headerTop}px`; // Only move UP
                logoField.style.left = '50vw'; // Stay centered horizontally
                logoField.style.transform = 'translate(-50%, 0)'; // Keep centered horizontally
                                    ...
                                    <div className="info-content flex flex-col gap-20 xsm:pb-50">
                                        ...
                                        <div className="copyright-field flex items-center justify-center">
                                            <p className="text text-[16px] xsm:text-14 font-normal text-white/60 [&_a]:duration-350 [&_a:hover]:text-primary md:text-center">
                                                ©2025 <Link href="/" className="uppercase">Seen Group</Link> | All Right reserved
                                            </p>
                                        </div>
                                    </div>
                                </div>
